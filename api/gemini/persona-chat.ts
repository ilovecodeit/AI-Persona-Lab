...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    console.log(`[PersonaChat] Calling Gemini (${GEMINI_CONFIG.MODEL}) for persona: ${personaId}`);
    ...
    console.error("[PersonaChat] Serverless function error:", error);
    const statusCode = error.status || error.statusCode || 500;
    return res.status(statusCode).json({
      error: error.message || "Gemini API 호출에 실패하였습니다.",
