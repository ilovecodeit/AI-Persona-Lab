...
import { getGeminiClient, withTimeout, GEMINI_CONFIG } from "./config";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    console.log(`[ExplainConcept] Calling Gemini (${GEMINI_CONFIG.MODEL}) for keyword: ${keyword}`);
    ...
    console.error("[ExplainConcept] Serverless function error:", error);
    const statusCode = error.status || error.statusCode || 500;
    return res.status(statusCode).json({
      error: error.message || "개념 분석 번역 도중 오류가 발생했습니다.",
