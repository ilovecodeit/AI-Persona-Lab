...
/**
 * Parses and returns structured error payload containing original status code and reason message
 */
export function handleError(error: any, context = "API") {
  console.error(`[${context} Error Details]`, error);
  
  // Extract real status code and message from Gemini SDK / Google API error or Fallback
  const statusCode = error.status || error.statusCode || 500;
  const message = error.message || "Gemini API 호출 중 알 수 없는 에러가 야기되었습니다.";
