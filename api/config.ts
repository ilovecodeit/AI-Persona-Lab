// Vercel Serverless Functions 및 Express Server가 참조할 수 있는 중앙 영구 설정 매니저
import { GoogleGenAI } from "@google/genai";

// 1. 핵심 전역 상수 관리
export const GEMINI_MODEL_NAME = "gemini-3.5-flash";
export const MAX_RETRY_COUNT = 2; // 일시적 연결 오류 시 최대 재시도 횟수
export const FETCH_TIMEOUT_MS = 8500; // Vercel 서버리스 함수 한도(Hobby 10초)를 고려한 안전 시간선

// 2. Persona System Prompts 수집
...
// 4. 안전한 타임아웃 및 재시도 실행 유틸리티
export async function runWithRetry<T>(fn: () => Promise<T>): Promise<T> {
  let attempt = 0;
  while (true) {
    try {
      // Promise.race를 이용해 지정한 타임아웃 내에 해결되지 않으면 에러가 나도록 설정
      const timeoutPromise = new Promise<never>((_, reject) => {
        setTimeout(() => reject(new Error("Gemini API Request Timeout exceeded.")), FETCH_TIMEOUT_MS);
      });

      return await Promise.race([fn(), timeoutPromise]);
    } catch (error: any) {
      attempt++;
      console.warn(`[Gemini API Warning] Attempt ${attempt} failed. Class: ${error?.name}. Message: ${error?.message}`);
      
      if (attempt >= MAX_RETRY_COUNT) {
        throw error; // 최대 재시도 횟수 소진 시 상위 핸들러로 에러 던짐
      }
      // 간단한 백오프 대기 후 재시도
