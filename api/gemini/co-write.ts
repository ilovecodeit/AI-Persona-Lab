...
위 조건들을 바탕으로 깊고 매력적인 시놉시스, 캐릭터 설정, 그리고 극의 시작을 알리는 명오프닝 장면에 대한 지시문과 대사 대본을 전문 스토리 작가처럼 작성해 주세요.`;
    ...
    console.log(`[CoWrite] Calling Gemini (${GEMINI_CONFIG.MODEL}) with genre: ${genre}`);
    ...
    console.error("[CoWrite] Serverless function error:", error);
    const statusCode = error.status || error.statusCode || 500;
    return res.status(statusCode).json({
      error: error.message || "공동 창작 스토리 빌딩 도중 오류가 발생했습니다.",
